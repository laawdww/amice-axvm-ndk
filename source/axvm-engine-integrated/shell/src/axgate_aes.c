...
static void add_round(uint8_t s[16], const uint8_t *rk)
{
    for (int i = 0; i < 16; ++i) {
        s[i] ^= rk[i];
    }
                                        ...
                                        const uint8_t *in, uint8_t *out, size_t len)
{
    ...
    axgate_secure_wipe(rk, sizeof(rk));
    axgate_secure_wipe(ctr, sizeof(ctr));
    axgate_secure_wipe(ks, sizeof(ks));
    return AXGATE_OK;
}
