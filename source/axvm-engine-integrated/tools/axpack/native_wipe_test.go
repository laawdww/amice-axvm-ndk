...
func TestBuildAXNWBlock(t *testing.T) {
	...
	blk := buildAXNWBlock(recs, seed)
	if len(blk) != 16+2*16 {
		t.Fatalf("len=%d", len(blk))
	}
	want := deriveAxnwMagic(seed)
	if got := uint32(blk[0]) | uint32(blk[1])<<8 | uint32(blk[2])<<16 | uint32(blk[3])<<24; got != want {
		t.Fatalf("magic=0x%x want=0x%x", got, want)
	}
	if want == axnwMagicLegacy {
		t.Fatal("derived magic should not equal legacy AXNW for this seed")
	}
}
